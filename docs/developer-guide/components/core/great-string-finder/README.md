...
```bash
pip install azul-great-string-finder
```

## Usage

A default entrypoint has been defined in `setup.py`, which will run `main()` in `azul_great_string_finder/train/main.py`  
There are two functions that can be called: train and tune.
Example: `azul-great-string-finder trainmodel RF f1 RS`, `azul-great-string-finder tune RF f1 RS`
The above example uses the parameters model = RF = RandomForest, score=f1, Search type = RS  
More details on these below.

```bash
$ azul-great-string-finder
...
`azul-great-string-finder tune RF accuracy GS`. Example of tuning the Random Forest model, with the score type accuracy, and search type GridSearch.
...
`azul-great-string-finder trainmodel` RF accuracy GS. Example of training the Random Forest model, with the score type accuracy, and search type GridSearch.
...
GSF = GreatStringFinder()
list_of_bools = GSF.find_great_strings(string_list)
